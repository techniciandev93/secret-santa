...
def bot_start(message: Message) -> None:
    ...
    try:
        ...
        game_instance = Game.objects.filter(uuid=uuid_param, start_registration_period__lte=current_time,
    ...
    if game_instance:
        ...
        bot.send_message(message.chat.id, f'Замечательно, ты собираешься участвовать в игре\n'
                                          ...
                                          f'{game_instance.start_registration_period.strftime("%Y-%m-%d %H:%M")} '
                                          f'- {game_instance.end_registration_period.strftime("%Y-%m-%d %H:%M")}\n'
